...
public:
  MonitorMutexTestThread2(Monitor<Mutex> &m) : _finished(false), _monitor(m) {}
  virtual void run() {
    Monitor<Mutex>::Lock lock(_monitor);
    _monitor.wait();
    _finished = true;
...
public:
  MonitorMutxTest() : testBase(monitorName) {}
...
void MonitorMutxTest::run() {
  ...
  test((t2->_finished && !t3->_finished) || (t3->_finished && !t2->_finished));
...
public:
  RecMutexTest() : testBase(recMutexName) {}
  virtual void run();
};
