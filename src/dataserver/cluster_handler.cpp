...
/* =======================Config===================== */
ClusterHandler::Config::Config() : info_(""), master_cs_addr_(""), slave_cs_addr_(""), group_name_(""), unit_("")
{
...
int ClusterHandler::init_conf(const std::string &info, int32_t timeout_ms, int32_t queue_limit)
{
    ...
    status_ = (ret == TAIR_RETURN_SUCCESS ? ClusterHandler::INITED : ClusterHandler::UNINIT);
    return ret;
}
...
{
    return config_ != NULL ? config_->cas_forbidden_namespaces(newnamespaces) : tair::common::Result<std::set<int> *>(NULL, TAIR_RETURN_FAILED);
}

tair::common::Result<std::set<int> *> ClusterHandler::cas_allowed_namespaces(std::set<int> *newnamespaces)
{
    return config_ != NULL ? config_->cas_allowed_namespaces(newnamespaces) : tair::common::Result<std::set<int> *>(NULL, TAIR_RETURN_FAILED);
}
...
{
    return config_ == NULL ? ClusterHandler::Config::DEFAULT_TIMEOUT : config_->timeout_ms_;
}

int ClusterHandler::queuelimit() const
{
    return config_ == NULL ? ClusterHandler::Config::DEFAULT_QUEUE_LIMIT : config_->queue_limit_;
}
...
{
    return make_pair(get_unit(), get_group_name());
}
