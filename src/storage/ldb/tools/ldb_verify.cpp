...
bool parse_args(int argc, char **argv, Configure *c)
{
    int opt;
...
bool parse_area_list(const char *s, Configure *c)
{
...
bool parse_bucket_list(const char *s, Configure *c)
{
    size_t size = strlen(s);
    const char *e = s + size;
    const char *p;
...
void parse_cluster_string(std::vector<std::string> &infos, const char *line,
...
void parse_rsync_local_cluster(Configure *c, const char *line)
{
...
void parse_rsync_remote_cluster(Configure *c, const char *line)
{
    std::vector<std::string> infos;
...
bool parse_rsync_info(Configure *c)
{
...
bool fill_bucket_map(Configure *c)
{
...
bool do_backup_db(tair_client_impl *client, uint64_t ds)
{
...
bool do_unload_db(tair_client_impl *client, uint64_t ds)
{
    //  std::vector<std::string> cmd_params;
    //  std::vector<std::string> infos;
...
bool download_local_file_create(Configure *c)
{
    // TODO
...
int data_write_local(Configure *c, const char *key, int keysize,
...
bool do_task(Task *t, Configure *c, int target_area)
{
...
void mark_key_interunit_or_innerunit_flag(Configure *c, data_entry *key)
{
...
int block_batch_put_util_success(Task *task, data_entry *key, data_entry *value, DestBucket *dbt)
{
    int t = 50 * 1000;
...
bool newest_file_finder(const string &s1, const string &s2)
{
    struct stat st1;
    struct stat st2;
    stat(s1.c_str(), &st1);
...
void do_statistics(int area, LdbKey &key, LdbItem &item)
{
...
int get_from_local(tair_client_impl &client, data_entry &key, data_entry *&value, bool &skip)
{
...
int get_from_slave(tair_client_impl &client, data_entry &key, data_entry *&value, bool &skip)
{
    skip = false;
    int ret = client.get_hidden(key.get_area(), key, value);
...
int get_from_remote(tair_client_impl &client, data_entry &key, data_entry *&value, bool &skip)
{
    return get_from_slave(client, key, value, skip);
}

bool verify_entry(bool remote, int area, const data_entry *key, const data_entry *local_value,
