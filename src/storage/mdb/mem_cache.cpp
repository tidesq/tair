...
int mem_cache::free_page(int slab_id)
{
    ...
    if ((slab_mng->free_pages_no + slab_mng->full_pages_no + slab_mng->partial_pages_no) <= 1)
    {
...
FREE_PAGE:
    ...
    assert(info != 0);
    assert(info->free_nr == slab_mng->per_slab && info->free_head != 0 && info->free_nr <= slab_mng->per_slab);
...
bool mem_cache::keep_area_quota(int area, uint64_t &exceeded_bytes)
{
    ...
    for (vector<slab_manager *>::iterator it = slab_managers.begin();
         it != slab_managers.end(); ++it)
    {
        if ((*it)->this_item_list[area].item_head == 0 || (*it)->this_item_list[area].item_tail == 0)
        {
...
bool mem_cache::slab_initialize()
{
    ...
    for (; i < cache_info->max_slab_id && start <= end; ++i)
    {
        ...
        if (start > static_cast<int>(end / 2))
        { //page_size slab
            start = end;
        }
        //get slabmng
        ...
        slabmng->partial_pages_bucket_num = (slabmng->per_slab + slab_manager::PARTIAL_PAGE_BUCKET - 1) / slab_manager::PARTIAL_PAGE_BUCKET;
...
mdb_item *mem_cache::slab_manager::evict_self(int &type)
{
    ...
    if (pos == 0)
    { // this area have no item of this slab
