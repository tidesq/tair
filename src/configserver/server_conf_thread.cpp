                                         ...
                                         uint64_t sync_config_server_id)
{
    ...
    {
        it->second->correct_server_info(sync_config_server_id && sync_config_server_id !=
    ...
    for (server_info_map::iterator sit = data_server_info_map.begin(); sit != data_server_info_map.end(); ++sit)
    {
        ...
        if (sit->second->status == server_info::ALIVE)
        {
            ...
            log_warn("HOST DOWN: %s lastTime is %u now is %u ",
                     tbsys::CNetUtil::addrToString(sit->second->server_id).c_str(), sit->second->last_time, heartbeat_curr_time);
...
void server_conf_thread::check_config_server_status(uint32_t loop_count)
{
    ...
    for (size_t i = 0; i < config_server_info_list.size(); i++)
    {
        ...
        if (server_info_tmp->server_id == util::local_server_ip::ip)
        { // this is myself
            continue;
        }
        if (server_info_tmp->last_time < now && server_info_tmp->status == server_info::ALIVE)
        { // downhost
            ...
            log_warn("CONFIG HOST DOWN: %s",
                     tbsys::CNetUtil::addrToString(server_info_tmp->server_id).c_str());
        }
        else if (server_info_tmp->last_time > now && server_info_tmp->status == server_info::DOWN)
        { // uphost
    ...
    {
        if (config_server_up && master_config_server_id == util::local_server_ip::ip)
        {
...
void server_conf_thread::load_config_server()
{
    ...
    for (size_t i = 0; i < str_list.size(); i++)
    {
        ...
        if (config_server_info_map.find(id) == config_server_info_map.end())
        {
            server_info *server_info =
                server_info_allocator::server_info_allocator_instance.new_server_info(NULL, id, eio, true);
    ...
    if (config_server_info_list.size() == 1U && config_server_info_list[0]->server_id ==
                                                    util::local_server_ip::ip)
    {
        master_config_server_id = util::local_server_ip::ip;
        return;
...
void server_conf_thread::run(tbsys::CThread *thread, void *arg)
{
    ...
    if (config_server_info_list.size() == 2U &&
        (config_server_info_list[0]->server_id == util::local_server_ip::ip || (sync_config_server_id == 0 && config_server_info_list[1]->server_id ==
                                                                                                                  util::local_server_ip::ip)))
    {
        master_config_server_id = util::local_server_ip::ip;
    ...
    while (!_stop)
    {
        ...
        for (size_t i = 0; i < config_server_info_list.size(); i++)
        {
            ...
            if (server_info_it->server_id == util::local_server_ip::ip)
                continue;
            if (server_info_it->status != server_info::ALIVE && (loop_count % 30) != 0)
            {
                                         ...
                                         response_get_group *resp)
{
    ...
    resp->set_hash_table(group_info_found->get_hash_table_deflate_data(req->server_flag),
                         group_info_found->get_hash_table_deflate_size(req->server_flag));
    ...
    change_server_list_to_group_file(request_data_server_ctrl *req, response_data_server_ctrl *resp)
{
...

    {
        ...
        {
            util::file_util::FileUtilChangeType op_type = (DATASERVER_CTRL_OP_DELETE == req->op_cmd ? util::file_util::CHANGE_FILE_DELETE : util::file_util::CHANGE_FILE_ADD);
                                              ...
                                              response_query_info *resp)
{
    ...
    case request_query_info::Q_MIG_INFO:
    {
        ...
        if (group_info_found->is_migrating())
        {
            ...
            for (vector<pair<uint64_t, uint32_t>>::iterator mig_it =
                     mig_machine_info.begin();
                 mig_it != mig_machine_info.end();
                 mig_it++)
            {
                snprintf(key, 32, "%s",
    ...
    case request_query_info::Q_DATA_SERVER_INFO:
    {
        ...
        {
            snprintf(key, 32, "%s",
                     tbsys::CNetUtil::addrToString((*node_it)->server->server_id).c_str());
                                             ...
                                             response_heartbeat *resp)
{
    ...
    if (req->config_version != group_info_founded->get_server_version() || // server version change
        (group_info_founded->is_transition_status() &&                     // transition version change
         (req->transition_info.transition_version != group_info_founded->get_transition_version() ||
          req->recovery_version != group_info_founded->get_recovery_version())))
    { // recovery version changed
        ...
        resp->set_hash_table(group_info_founded->get_hash_table_deflate_data(req->server_flag),
                             group_info_founded->get_hash_table_deflate_size(req->server_flag));
    ...
    if (req->area_capacity_version != resp->area_capacity_version)
    {
        resp->set_area_capacity(group_info_founded->get_area_capacity_info(),
                                ...
                                group_info_founded->get_available_server_id().size());
    }
    ...
    if (req->pull_migrated_info != 0)
    {
             ...
             ++i)
        {
            if (req->get_server_id() == p_s_table[i])
            {
                ...
                {
                    resp->migrated_info.push_back(p_m_table
                                                      [i +
                                                       j *
                                                     ...
                                                     req)
{
    ...
    server_info_map::iterator it =
        data_server_info_map.find(req->server_id);
    if (it != data_server_info_map.end() && it->second->group_info_data != NULL)
    {
        if (it->second->group_info_data->do_finish_migrate(req->server_id, req->version, req->bucket_no,
...
int server_conf_thread::do_finish_recovery_packet(request_recovery_finish *req)
{
    ...
    if (it != data_server_info_map.end() && it->second->group_info_data != NULL)
    {
        if (it->second->group_info_data->do_finish_recovery(req->server_id, req->version, req->transition_version, req->bucket_no) == false)
        {
            if (ret == 0)
            {
                ret = -1;
                                                      ...
                                                      int value)
{
    ...
    log_info("get_master_config_server id is [%ld], ip=%s",
...
void server_conf_thread::table_builder_thread::build_table(vector<
                                                           group_info *> &groupe_will_builded)
{
