...
group_info::group_info(const char *p_group_name,
                       server_info_map *p_server_info_map, easy_io_t *eio) {
  this->eio = eio;
  ...
  const char *sz_data_dir = TBSYS_CONFIG.getString(
      CONFSERVER_SECTION, TAIR_DATA_DIR, TAIR_DEFAULT_DATA_DIR);
  snprintf(server_table_file_name, 256, "%s/%s_server_table", sz_data_dir,
           group_name);
  ...
  for (uint32_t i = 0; i < server_table_manager.get_server_bucket_count();
    ...
    if (server_table_manager.m_hash_table[i] != 0) {
      if (server_table_manager.m_hash_table[i] !=
        ...
        for (uint32_t j = 1; j < server_table_manager.get_copy_count(); j++) {
          bool found = false;
          for (uint32_t k = 1; k < server_table_manager.get_copy_count(); k++) {
            if (server_table_manager.d_hash_table
                    [j * server_table_manager.get_server_bucket_count() + i] ==
                server_table_manager.m_hash_table
                    [k * server_table_manager.get_server_bucket_count() + i]) {
              found = true;
              break;
            }
          }
      ...
      if (migrate_this_bucket) {
        ...
        migrate_count++;
        log_info(
            "added migrating machine: %s bucket %d ",
            tbsys::CNetUtil::addrToString(server_table_manager.m_hash_table[i])
                .c_str(),
            i);
      }
    }
...
const char *group_info::get_group_name() const { return group_name; }

tbsys::STR_STR_MAP *group_info::get_common_map() { return &common_map; }
...
int group_info::get_hash_table_deflate_size(int mode) const {
  ...
  return server_table_manager.hash_table_deflate_data_for_data_server_size;
}

// server_list=ip:port,ip:port;...
void group_info::parse_server_list(node_info_set &list, const char *server_list,
  ...
  int default_port = TBSYS_CONFIG.getInt(TAIRSERVER_SECTION, TAIR_PORT,
  ...
  for (uint32_t i = 0; i < node_list.size(); i++) {
    ...
    for (uint32_t j = 0; j < ip_list.size(); j++) {
      ...
      if (it == server_info_maps->end()) {
        sinfo = server_info_allocator::server_info_allocator_instance
...
void group_info::parse_plugins_list(vector<const char *> &plugins) {
  ...
  for (vector<const char *>::iterator it = plugins.begin(); it != plugins.end();
       it++) {
...
void group_info::parse_area_key_list(vector<const char *> &a_c) {
  ...
  for (vector<const char *>::iterator it = a_c.begin(); it != a_c.end(); it++) {
  ...
  if (area_key_info.size() != area_key_info_tmp.size()) {
    ...
    for (map<std::string, uint32_t>::iterator new_it =
      ...
      if (old_it == area_key_info.end() || new_it->second != old_it->second) {
        need_update = true;
        break;
      }
    }
...
void group_info::parse_area_capacity_list(vector<const char *> &a_c) {
  ...
  for (vector<const char *>::iterator it = a_c.begin(); it != a_c.end(); it++) {
    ...
    for (uint32_t i = 0; i < info.size(); i++) {
      ...
      if (p) {
        ...
        log_debug("area %u capacity %lu", area, capacity);
        set_bitmap_area(area);
      }
    }
  }
  bool need_update = false;
  if (always_update_capacity_info ||
      (area_capacity_info.size() != area_capacity_info_tmp.size())) {
    need_update = true;
  } else {
  ...
  if (need_update) {
    ...
    for (; it != area_capacity_info.end(); it++) {
      log_info("set area %u capacity is %lu", it->first, it->second);
    }
    inc_version(server_table_manager.area_capacity_version);
  }
}
...
uint32_t group_info::find_available_area(const char *group_file_name,
...
int group_info::add_area_key_list(const char *group_file_name,
                                  const char *group_name, const char *area_key,
                                  uint32_t area_num) {
...
int group_info::add_area_capacity_list(const char *group_file_name,
                                       const char *group_name,
                                       const char *area_key, uint32_t &area_num,
                                       const char *quota) {
  int ret = TAIR_RETURN_FAILED;
  ...
  if (strlen(area_key) == 5 &&
      strncmp(area_key, "65536", 5) ==
          0) { // if area_key == 65536, find available_area
  ...
  } else if (it != area_key_info
                       .end()) { // if key exist, modify quota or delete qutao
  ...
  if (TAIR_RETURN_SUCCESS == ret) {
    ...
    if (quota_num > 0) {
      area_capacity_info.insert(
          std::pair<uint32_t, uint64_t>(new_area_num, quota_num));
    }
...
int group_info::update_area_capacity_list(const char *group_file_name,
                                          const char *group_name,
                                          uint32_t area_num,
                                          const char *quota) {
  int ret = TAIR_RETURN_FAILED;
  ...
  if (it !=
      area_capacity_info.end()) { // if key exist, modify quota or delete qutao
    snprintf(key, sizeof(key), TAIR_STR_AREA_CAPACITY_LIST "=%d", area_num);
    if (strlen(quota) == 1 &&
        quota[0] == '0') { // if quota == "0", remove this quota
      util::file_util::change_conf(group_file_name, group_name, key, value,
                                   util::file_util::CHANGE_FILE_DELETE, ',');
      snprintf(key, sizeof(key), TAIR_STR_AREA_KEY_LIST "=%d", area_num);
      snprintf(value, sizeof(value), "%d;", area_num);
      ret = util::file_util::change_conf(
          group_file_name, group_name, key, value,
          util::file_util::CHANGE_FILE_DELETE, ',');
...
void group_info::correct_server_info(bool is_sync) {
  ...
  // get dest hash table. all server avialbe should in this table. we believe
    ...
    for (sit = node_list.begin(); sit != node_list.end(); ++sit) {
      ...
      if (it == live_machines.end()) {
        if (is_sync) {
          ...
          // illegal server
          log_warn(
              "set server %s status is down",
              tbsys::CNetUtil::addrToString((*sit)->server->server_id).c_str());
...
bool group_info::load_config(tbsys::CConfig &config, uint32_t version,
  ...
  data_need_move = config.getInt(group_name, TAIR_STR_GROUP_DATA_NEED_MOVE,
                                 TAIR_DATA_NEED_MIGRATE);
  int bucket_count = config.getInt(group_name, TAIR_STR_BUCKET_NUMBER,
                                   TAIR_DEFAULT_BUCKET_NUMBER);
  int copy_count =
      config.getInt(group_name, TAIR_STR_COPY_COUNT, TAIR_DEFAULT_COPY_COUNT);
  if (!server_table_manager.is_file_opened()) {
    char file_name[256];
    const char *sz_data_dir = TBSYS_CONFIG.getString(
        CONFSERVER_SECTION, TAIR_DATA_DIR, TAIR_DEFAULT_DATA_DIR);
    snprintf(file_name, 256, "%s/%s_server_table", sz_data_dir, group_name);

    bool tmp_ret =
        server_table_manager.create(file_name, bucket_count, copy_count);
    if (!tmp_ret) {
      log_error("create file: %s failed, bucket_count: %d, copy_count: %d",
                file_name, bucket_count, copy_count);
      exit(1);
    }

    log_info("set bucket count = %u ok",
             server_table_manager.get_server_bucket_count());
    log_info("set copy count = %u ok", server_table_manager.get_copy_count());
  }
  ...
  min_config_version = config.getInt(group_name, TAIR_STR_MIN_CONFIG_VERSION,
  ...
  server_down_time =
      config.getInt(group_name, TAIR_STR_SERVER_DOWNTIME, TAIR_SERVER_DOWNTIME);
  ...
  float f_min_data_server_count = strtof(
      config.getString(group_name, TAIR_STR_MIN_DATA_SRVER_COUNT, "0"), NULL);
  ...
  log_info(" %s = %d", TAIR_STR_MIN_DATA_SRVER_COUNT, min_data_server_count);

  int old_build_strategy = build_strategy;
  build_strategy =
      config.getInt(group_name, TAIR_STR_BUILD_STRATEGY, TAIR_BUILD_STRATEGY);

  // modify accept_strategy & lost_tolerance_flag : no need to rebulid table
  accept_strategy = static_cast<GroupAcceptStrategy>(
      config.getInt(group_name, TAIR_STR_ACCEPT_STRATEGY,
                    static_cast<int>(GROUP_DEFAULT_ACCEPT_STRATEGY)));
  lost_tolerance_flag = static_cast<DataLostToleranceFlag>(config.getInt(
      group_name, TAIR_STR_DATALOST_FLAG, static_cast<int>(NO_DATA_LOST_FLAG)));

  bucket_place_flag = config.getInt(group_name, TAIR_STR_BUCKET_DISTRI_FLAG, 0);

  log_info("build_strategy: %d, accept_strategy: %d, lost_tolerance_flag: %d, "
           "bucket_place_flag: %d",
           build_strategy, static_cast<int>(accept_strategy),
           static_cast<int>(lost_tolerance_flag), bucket_place_flag);

  float old_ratio = diff_ratio;
  diff_ratio = strtof(config.getString(group_name, TAIR_STR_BUILD_DIFF_RATIO,
                                       TAIR_BUILD_DIFF_RATIO),
                      NULL);
  uint64_t old_pos_mask = pos_mask;
  pos_mask =
      strtoll(config.getString(group_name, TAIR_STR_POS_MASK, "0"), NULL, 10);
  if (pos_mask == 0)
    pos_mask = TAIR_POS_MASK;
  log_info(" %s = %lX", TAIR_STR_POS_MASK, pos_mask);

  always_update_capacity_info =
      config.getInt(group_name, TAIR_ALWAYS_UPDATE_CAPACITY_INFO, 0) > 0;
  allow_failover_server =
      config.getInt(group_name, TAIR_ALLOW_FAILOVER_SERVER, 0) > 0;

  pre_load_flag = config.getInt(group_name, TAIR_PRE_LOAD_FLAG, 0);
  const char *down_servers =
      config.getString(group_name, TAIR_TMP_DOWN_SERVER, "");
  log_debug("pre load flag: %d", pre_load_flag);
  log_debug("down servers: %s", down_servers);
  if (pre_load_flag == 0) {
    if (strlen(down_servers) != 0) {
      log_error(
          "no %s config or config 0, tmp down server %s config is ignored",
          TAIR_PRE_LOAD_FLAG, down_servers);
    }
    tmp_down_server.clear();
  } else {
  ...
  if (*server_table_manager.last_load_config_time >= version) {
    log_info("[%s] config file unchanged, load config count: %d", group_name,
             load_config_count);
  ...
  interval_seconds = config.getInt(group_name, TAIR_STR_REPORT_INTERVAL,
  ...
  *server_table_manager.last_load_config_time = version;
  ...
  float min =
      old_ratio > diff_ratio ? old_ratio - diff_ratio : diff_ratio - old_ratio;
  ...
  if (changed1 || changed2) {
    ...
    if (changed2) {
      ...
      for (std::map<uint64_t, node_stat_info>::iterator stat_it =
               stat_info.begin();
           stat_it != stat_info.end();) {
        tmp_node_info.server->server_id = stat_it->first;
        // This stat info is not needed any more.
        // NOTE: We only clear stat info here, 'cause this ds has been removed
        // from _server_list,
        //       we do nothing when ds is DOWN (maybe also clear).
        if (node_list.find(&tmp_node_info) == node_list.end()) {
          log_info("erase stat info of server: %s",
                   tbsys::CNetUtil::addrToString(stat_it->first).c_str());
...
void group_info::failover_server(node_info_set &upnode_list, bool &change) {
  ...
  if (!allow_failover_server || server_table_manager.get_copy_count() < 2 ||
      is_migrating()) {
  ...
  for (it = recovery_machine.begin(); it != recovery_machine.end(); ++it) {
    log_debug("add server %s to recovery_ds",
  ...
  for (std::set<uint64_t>::const_iterator it = pending_failover_servers.begin();
       it != pending_failover_servers.end(); ++it) {
    ...
    if (iter != recovery_servers.end()) {
      log_warn("current recover server %s down",
               tbsys::CNetUtil::addrToString(server_id).c_str());
  ...
  (*server_table_manager.recovery_block_count) =
      recover_count > 0 ? recover_count : -1;

  // check_failovering to modify failovring status
  // if pending_failover_server hold some buckets still used, failovring likely
  // to be true
  check_failovering();
  pending_failover_servers.clear();
  log_debug("@@ after failover: %d %d", failovering ? 1 : 0,
            *server_table_manager.transition_version);
}

bool group_info::failover_one_ds(uint64_t server_id, node_info_set &upnode_list,
                                 bool &change, bool check_alive) {
  ...
  do {
    ...
    if (check_alive && upnode_list.find(&ninfo) != upnode_list.end()) {
      log_warn("failover server %s but alive now",
               tbsys::CNetUtil::addrToString(server_id).c_str());
      continue;
    }

    log_info("failover server: %s",
             tbsys::CNetUtil::addrToString(server_id).c_str());
        ...
        if (c == 0) {
          ...
          for (++c; c < copy_count; ++c) {
            if (hash_table[bucket + c * bucket_count] != 0) {
              hash_table[bucket] = m_hash_table[bucket] =
                  hash_table[bucket + c * bucket_count];
              hash_table[bucket + c * bucket_count] =
                  m_hash_table[bucket + c * bucket_count] = 0;
    ...
    if (!is_server_failover) {
      log_error(
          "## server(%s) is down, but hold no bucket, then no failover...",
          tbsys::CNetUtil::addrToString(server_id).c_str());
    }
  } while (false);
...
void group_info::recover_server(node_info_set &upnode_list, bool &change) {
  ...
  if (!failovering || server_table_manager.get_copy_count() < 2 ||
      is_migrating()) {
  ...
  for (std::set<uint64_t>::const_iterator it = pending_recover_servers.begin();
       it != pending_recover_servers.end(); ++it) {
...
void group_info::recover_one_ds(uint64_t server_id, node_info_set &upnode_list,
                                bool &change, set<uint64_t> &recovery_servers) {
  server_info sinfo;
  sinfo.server_id = server_id;
  node_info ninfo(&sinfo);
  ...
  if (upnode_list.find(&ninfo) == upnode_list.end()) {
    log_warn("recover server %s but not alive now",
             tbsys::CNetUtil::addrToString(server_id).c_str());
    return;
  }
  ...
  if (failovering_servers.find(server_id) != failovering_servers.end()) {
    ...
    log_warn("recover server(%s) detect, remove from failovering_servers and "
             "add into recovery_servers",
             tbsys::CNetUtil::addrToString(server_id).c_str());
    change = true;
  } else {
...
int group_info::fill_recovery_machine(set<uint64_t> &recovery_servers) {
  ...
  if (copy_count != 2) {
    log_error("@error, copy_count isn't 2, but enter the failover process, "
              "check configuration");
    return 0;
  }
  ...
  for (int32_t b = 0; b < bucket_count; ++b) {
    ...
    failover_server =
        (backup_server == d_hash_table[b] ? d_hash_table[bucket_count + b]
                                          : d_hash_table[b]);
...
void group_info::check_failovering() {
  if (server_table_manager.get_copy_count() < 2 || is_migrating()) {
    failovering = false;
  } else {
    uint64_t *hash_table = server_table_manager.get_hash_table();
    uint64_t *m_hash_table = server_table_manager.get_m_hash_table();
    ...
    failovering = memcmp(hash_table, d_hash_table, table_size) != 0 ||
...
int group_info::force_migrate_bucket(uint32_t bucket_no, uint32_t copy_index,
                                     uint64_t src_server_id,
  ...
  if (ret == TAIR_RETURN_SUCCESS &&
      (available_server.find(src_server_id) == available_server.end() ||
       available_server.find(dest_server_id) == available_server.end())) {
    log_error("server is not exist in available server. src_server: %s, "
              "dest_server: %s",
  ...
  if (ret == TAIR_RETURN_SUCCESS) {
    int bucket_index =
        bucket_no + copy_index * server_table_manager.get_server_bucket_count();
    // check src_server_id again
    if (server_table_manager.m_hash_table[bucket_index] != src_server_id) {
      log_error(
          "src server is wrong. server in m_hash_table: %s, src_server: %s",
      ...
      for (uint32_t i = 0; i < server_table_manager.get_copy_count(); ++i) {
        if (i != copy_index) {
          int tmp_bucket_index =
              bucket_no + i * server_table_manager.get_server_bucket_count();
          if (server_table_manager.m_hash_table[tmp_bucket_index] ==
              dest_server_id) {
            log_error("dest server is already hold this bucket. line: %d, "
                      "server in m_hash_table: %s, dest_server: %s",
                      i,
                      tbsys::CNetUtil::addrToString(
                          server_table_manager.m_hash_table[tmp_bucket_index])
                          .c_str(),
                      tbsys::CNetUtil::addrToString(dest_server_id).c_str());
            ret = TAIR_RETURN_FAILED;
            break;
    ...
    if (ret == TAIR_RETURN_SUCCESS) {
      log_warn(
          "d_hash_table, bucket_index: %d, src_server: %s, dest_server: %s",
...
int group_info::force_switch_bucket(set<uint32_t> bucket_nos,
                                    tbsys::CRWSimpleLock *p_grp_locker,
                                    tbsys::CRWSimpleLock *p_server_locker) {
  // hold all lock
  node_info_set upnode_list;
  ...
  // check bucket num
  if (ret == TAIR_RETURN_SUCCESS) {
    for (set<uint32_t>::const_iterator iter = bucket_nos.begin();
         iter != bucket_nos.end(); iter++) {
      uint32_t bucket_no = *iter;
      if (bucket_no >= server_table_manager.get_server_bucket_count()) {
        log_error("%s failed as bucket_no is illegal. bucket_no: %d, "
                  "server_bucket_count: %d",
                  option.ops_name, bucket_no,
                  server_table_manager.get_server_bucket_count());
        ret = TAIR_RETURN_FAILED;
        break;
      }
  ...
  if (ret == TAIR_RETURN_SUCCESS) {
    for (set<uint32_t>::const_iterator iter = bucket_nos.begin();
         iter != bucket_nos.end(); iter++) {
      uint32_t master_index = *iter,
               slave_index = master_index +
                             server_table_manager.get_server_bucket_count();
      ...
      if (server_table_manager.m_hash_table[master_index] == 0 ||
        ...
        log_error("%s failed as bucket %d holds less than two copies, master "
                  "is %s, slave is %s",
                  option.ops_name, master_index,
                  tbsys::CNetUtil::addrToString(
                      server_table_manager.m_hash_table[master_index])
                      .c_str(),
                  tbsys::CNetUtil::addrToString(
                      server_table_manager.m_hash_table[slave_index])
                      .c_str());
        ret = TAIR_RETURN_FAILED;
        break;
      }
      ...
      if (server_table_manager.m_hash_table[master_index] !=
              server_table_manager.d_hash_table[master_index] ||
          server_table_manager.m_hash_table[slave_index] !=
              server_table_manager.d_hash_table[slave_index]) {
        log_error("%s failed as bucket %d, m_hashtable diff from d_hashtable",
                  option.ops_name, master_index);
        ret = TAIR_RETURN_FAILED;
        break;
      }
...
int group_info::active_failover_ds(uint64_t server_id,
  ...
  if (ret == TAIR_RETURN_SUCCESS) {
    bool changed = false,
         is_server_failover =
             failover_one_ds(server_id, upnode_list, changed, false);
    check_failovering();
...
int group_info::active_recover_ds(uint64_t server_id,
  ...
  if (ret == TAIR_RETURN_SUCCESS &&
      failovering_servers.find(server_id) == failovering_servers.end()) {
    log_error("%s failed as active recover server %s, but it's not in failover",
              option.ops_name,
              tbsys::CNetUtil::addrToString(server_id).c_str());
    ret = TAIR_RETURN_FAILED;
  }
  ...
  if (ret == TAIR_RETURN_SUCCESS) {
    ...
    if (!recovery_servers.empty()) {
      int recover_count = fill_recovery_machine(recovery_servers);
      if (recover_count > 0) {
        log_error("active recover %s ok, recovery begin",
                  tbsys::CNetUtil::addrToString(server_id).c_str());
        log_warn("need recovery, count: %d", recover_count);
      }
      (*server_table_manager.recovery_block_count) =
    ...
    for (int i = 0; i < bucket_count && ret == TAIR_RETURN_SUCCESS; i++) {
      for (int j = 0; j < copy_count; j++) {
        ...
        if (dst_ds_set.find(server_id) != dst_ds_set.end()) {
          log_error("dst server(%s) hold bucket already",
                    tbsys::CNetUtil::addrToString(server_id).c_str());
          ret = TAIR_RETURN_FAILED;
          break;
        }
  ...
  if (ret == TAIR_RETURN_SUCCESS) {
    if (target_items.empty()) {
      ...
      for (; set_iter != target_items.end(); set_iter++) {
        ...
        map<uint64_t, uint64_t>::const_iterator map_iter =
            replace_ds_pairs.find(src_server_id);
...
int group_info::urgent_offline(uint64_t offline_id,
                               std::set<uint64_t> replace_ids,
                               tbsys::CRWSimpleLock *p_grp_locker,
                               tbsys::CRWSimpleLock *p_server_locker) {
  // hold all lock
  node_info_set upnode_list;
  ...
  if (ret == TAIR_RETURN_SUCCESS) {
    for (set<uint64_t>::const_iterator iter = replace_ids.begin();
         iter != replace_ids.end(); iter++) {
  ...
  if (ret == TAIR_RETURN_SUCCESS && single_copy &&
    ...
    log_error("%s failed as group in abnormal status when copy count is 1",
              option.ops_name);
    ret = TAIR_RETURN_FAILED;
  }
  ...
  if (ret == TAIR_RETURN_SUCCESS) {
    if ((*server_table_manager.migrate_block_count) != -1) {
      ...
      if (available_server.find(offline_id) != available_server.end()) {
        log_error("%s failed when group is migrating as urgent offline server "
                  "%s is still alive",
                  option.ops_name,
      ...
      if (failovering_servers.find(offline_id) == failovering_servers.end()) {
        log_error("%s failed when group is failovering as urgent offline "
                  "server %s, but it's not in failover",
                  option.ops_name,
                  tbsys::CNetUtil::addrToString(offline_id).c_str());
        ret = TAIR_RETURN_FAILED;
      }
      ...
      // if down, need to change hash_table and m_hashtable, if not alive, just
      // change hash_table
      if (available_server.find(offline_id) == available_server.end()) {
        need_quick_kick = true;
      }
      log_info("urgent offline when normal status, offline_id is %s",
               need_quick_kick ? "dead" : "alive");
    }
  }
  ...
  if (ret == TAIR_RETURN_SUCCESS && !single_copy) {
    if (need_quick_kick) {
      tmp_mhashtable =
          (uint64_t *)malloc(bucket_count * copy_count * sizeof(uint64_t));
      memcpy(tmp_mhashtable, m_hash_table,
             bucket_count * copy_count * sizeof(uint64_t));
    }
    ...
    for (int32_t b = 0; b < bucket_count; ++b) {
      ...
      if (got && valid_copy == 1) {
        log_error(
            "can not urgent offline %s, bucket %d has the last copy here.",
  ...
  if (ret == TAIR_RETURN_SUCCESS) {
    tmp_dhashtable =
        (uint64_t *)malloc(bucket_count * copy_count * sizeof(uint64_t));
    memcpy(tmp_dhashtable, d_hash_table,
           bucket_count * copy_count * sizeof(uint64_t));
    ...
    for (int i = 0; i < bucket_count; i++) {
      ...
      if (tmp_dhashtable[i] == offline_id) {
        ...
        partner_id = single_copy ? 0 : tmp_dhashtable[i + bucket_count];
                 ...
                 tmp_dhashtable[i + bucket_count] == offline_id) {
        got = true;
      ...
      if (got) {
        ...
        if (vec_replace_ids[j] == partner_id) {
          ...
          while (true) {
            ...
            if (k == j) {
              log_error(
                  "%s failed as bucket %d is hold by (%s, %s), can't be "
                  "replaced",
                  option.ops_name, i,
                  tbsys::CNetUtil::addrToString(tmp_dhashtable[i]).c_str(),
                  tbsys::CNetUtil::addrToString(
                      tmp_dhashtable[i + bucket_count])
                      .c_str());
              valid = false;
              break;
            }
    ...
    if (valid) {
      ...
      *server_table_manager.recovery_block_count = -1;
      ...
      if (need_quick_kick) {
        uint64_t *hash_table = server_table_manager.get_hash_table();
        memcpy(hash_table, tmp_mhashtable,
               bucket_count * copy_count * sizeof(uint64_t));
        memcpy(m_hash_table, tmp_mhashtable,
               bucket_count * copy_count * sizeof(uint64_t));
      }
      memcpy(d_hash_table, tmp_dhashtable,
             bucket_count * copy_count * sizeof(uint64_t));
...
int group_info::force_table_ops_check(const ops_check_option &option) {
  ...
  do {
    // check migrating
    if (option.check_migrate_down &&
        (*server_table_manager.migrate_block_count) != -1) {
      log_error("%s failed as server is migrating. migrate block count: %d",
                option.ops_name, *(server_table_manager.migrate_block_count));
      ret = TAIR_RETURN_FAILED;
      break;
    }
    ...
    if (option.check_failover_up && !failovering) {
      log_error("%s failed as group is not in failovering status",
                option.ops_name);
      ret = TAIR_RETURN_FAILED;
      break;
    }
    ...
    if (2 != server_table_manager.get_copy_count() &&
        (!option.allow_single_copy ||
         1 != server_table_manager.get_copy_count())) {
      log_error("%s failed as copy_count is %d, allow_single_copy is %d",
                option.ops_name, server_table_manager.get_copy_count(),
                option.allow_single_copy);
      ret = TAIR_RETURN_FAILED;
      break;
    }
    ...
    if (option.check_min_ds_num) {
      ...
      if (option.upnode_list_size >=
          static_cast<size_t>(min_data_server_count)) {
        log_error("%s failed as min_server_count(%d) is not larger than "
                  "alive_server_num(%lu)",
                  option.ops_name, min_data_server_count,
                  option.upnode_list_size);
        ret = TAIR_RETURN_FAILED;
        break;
      }
    }
...
void group_info::rebuild(uint64_t slave_server_id,
                         tbsys::CRWSimpleLock *p_grp_locker,
                         tbsys::CRWSimpleLock *p_server_locker) {
  log_warn("start rebuild table of %s, build_strategy: %d", group_name,
           build_strategy);
  node_info_set upnode_list;
  table_builder::hash_table_type quick_table_tmp;
  table_builder::hash_table_type
      hash_table_for_builder_tmp; // will put rack info to this
  ...
  if (size <= (int)min_data_server_count) {
    ...
    if (size < (int)min_data_server_count) {
      ...
      // eg. start cluster first time, always set min_data_server equal server
      // count
      need_rebuild = !failover_change && !recover_change;
    }

    log_debug("@@ need rebuild %d, failchange %d, recover change %d",
              need_rebuild, failover_change, recover_change);
    if (!need_rebuild) {
  ...
  *server_table_manager.recovery_block_count = -1;
  ...
  if (build_strategy == 1) {
    ...
    p_table_builder = new table_builder2(
        diff_ratio, server_table_manager.get_server_bucket_count(),
        server_table_manager.get_copy_count());
  ...
  int ret = p_table_builder->rebuild_table(
      hash_table_for_builder_tmp, dest_hash_table_for_builder_tmp, true);
  gettimeofday(&t_end, NULL);
  log_error("rebuild_table consume time : %lu us",
            1000000 * (t_end.tv_sec - t_start.tv_sec) + t_end.tv_usec -
                t_start.tv_usec);
  // p_grp_locker->wrlock();
  if (ret == 0) {
  ...
  if ((data_need_move == TAIR_DATA_NEED_MIGRATE) && first_run == false) {
    ...
    if (server_table_manager.get_copy_count() <= 1) {
      ...
      for (uint32_t i = 0; i < server_table_manager.get_server_bucket_count();
           ++i) {
        // if master bucket is not available, use d_hash_table to fill
        // m_hash_table
        if (available_server.find(server_table_manager.m_hash_table[i]) ==
            available_server.end()) {
          log_info(
              "index %u:%s in m_hash_table is invalid, use d_hash_table: %s", i,
              tbsys::CNetUtil::addrToString(
                  server_table_manager.m_hash_table[i])
                  .c_str(),
              tbsys::CNetUtil::addrToString(
          ...
          for (uint32_t j = 0; j < server_table_manager.get_copy_count(); ++j) {
            uint32_t bucket_index =
                i + j * server_table_manager.get_server_bucket_count();
            server_table_manager.m_hash_table[bucket_index] =
  ...
  for (uint32_t i = 0; i < server_table_manager.get_server_bucket_count() *
    ...
    log_info("+++ line: %d bucket: %d => %s",
             ...
             i % server_table_manager.get_server_bucket_count(),
             tbsys::CNetUtil::addrToString(server_table_manager.hash_table[i])
                 .c_str());
  ...
  for (uint32_t i = 0; i < server_table_manager.get_server_bucket_count() *
    ...
    log_info("+++ line: %d bucket: %d => %s",
             i / server_table_manager.get_server_bucket_count(),
             i % server_table_manager.get_server_bucket_count(),
             tbsys::CNetUtil::addrToString(server_table_manager.d_hash_table[i])
                 .c_str());
#endif
  ...
  // migration is a process which move data based on _p_m_hashTable to data
  // based on _p_d_hashTable when the migration is finished _p_hashTable will be
  // covered by _p_d_hashTable, so client can use the new table but if something
  // interrupt the migrationg process, for example lost a data server, we will
  // use _p_m_hashTable to build a new _p_d_hashTable. data server will receive
  // _p_hashTable and  _p_d_hashTable so they know how to start a migration.
  // client will receive _p_hashTable they will use this table to decide which
  // data server they should ask for a key.
  int diff_count = 0;
  if (first_run == false) {
  ...
  inc_version(server_table_manager.client_version);
  inc_version(server_table_manager.server_version);
  inc_version(server_table_manager
                  .area_capacity_version); // data server count changed, so
                                           // capacity for every data server
  ...
  server_table_manager.sync();
  {
    ...
    for (uint32_t i = 0; i < server_table_manager.get_server_bucket_count();
      ...
      for (uint32_t j = 0; j < server_table_manager.get_copy_count(); j++) {
        ...
        for (uint32_t k = 0; k < server_table_manager.get_copy_count(); ++k) {
          if (server_table_manager.d_hash_table
                  [i + server_table_manager.get_server_bucket_count() * j] ==
              server_table_manager.m_hash_table
                  [i + server_table_manager.get_server_bucket_count() * k]) {
            migrate_it = false;
            break;
          }
        }
  ...
  log_info("[%s] different ratio is : migrateCount:%d/%d=%.2f%%", group_name,
           diff_count,
           server_table_manager.get_server_bucket_count() *
               server_table_manager.get_copy_count(),
           100.0 * diff_count /
...
bool group_info::do_finish_migrate(uint64_t server_id, uint32_t server_version,
                                   int bucket_id, uint64_t slave_server_id) {
  if (data_need_move != TAIR_DATA_NEED_MIGRATE)
    return false;
  log_info("migrate finish: from[%s] base version: [%d], now version: [%d] ",
           tbsys::CNetUtil::addrToString(server_id).c_str(), server_version,
           *server_table_manager.server_version);
  if (server_version != *server_table_manager.server_version) {
    log_error("migrate version conflict, ds version: %d, cs version: %d",
              server_version, *server_table_manager.server_version);
    return false;
  }
...
bool group_info::do_finish_recovery(uint64_t server_id, uint32_t server_version,
                                    ...
                                    int bucket_id) {
  log_info("@@ recovery finish: from[%s] base version: [%d:%d], now version: "
           "[%d:%d], bucket_id: [%d] ",
           tbsys::CNetUtil::addrToString(server_id).c_str(), server_version,
           transition_version, *server_table_manager.server_version,
           *server_table_manager.transition_version, bucket_id);

  if (server_version != *server_table_manager.server_version) {
    log_error(
        "recovery server version conflict, ds version: %d, cs version: %d",
        server_version, *server_table_manager.server_version);
    return false;
  }
  ...
  if (transition_version != *server_table_manager.transition_version) {
    log_error(
        "recovery transition version conflict, ds version: %d, cs version: %d",
        transition_version, *server_table_manager.transition_version);
    return false;
  }
...
bool group_info::check_migrate_complete(uint64_t slave_server_id,
  ...
  if (migrate_machine.size() > 0U) {
    ...
    log_info("[%s] still have %lu data server are in migrating.", group_name,
             migrate_machine.size());
    if (TBSYS_LOGGER._level >= TBSYS_LOG_LEVEL_DEBUG) {
      for (it = migrate_machine.begin(); it != migrate_machine.end(); it++) {
        log_debug("server %s still have %d buckets waiting to be migrated",
...
bool group_info::check_recovery_complete(uint64_t slave_server_id,
                                         tbsys::CRWSimpleLock *p_grp_locker) {
  p_grp_locker->rdlock();
  if ((*server_table_manager.recovery_block_count) == -1) {
    p_grp_locker->unlock();
  ...
  if (recovery_machine.size() > 0U) {
    log_info("[%s] still have %lu data server are in recovering.", group_name,
             recovery_machine.size());
    map<uint64_t, int>::iterator it;
    if (TBSYS_LOGGER._level >= TBSYS_LOG_LEVEL_DEBUG) {
      for (it = recovery_machine.begin(); it != recovery_machine.end(); it++) {
        log_debug("server %s still have %d buckets waiting to be recovered",
                  tbsys::CNetUtil::addrToString(it->first).c_str(), it->second);
      }
    }
    if (should_syn_mig_info) {
  ...
  // just check if failovering is over, in that case, we reset transistion
  // version
...
void group_info::set_force_rebuild() { need_rebuild_hash_table = 1; }
...
void group_info::print_server_count() {
  ...
  for (hash_map<uint64_t, int>::iterator it = m.begin(); it != m.end(); ++it) {
...
void group_info::send_server_table_packet(uint64_t slave_server_id) {
  ...
  if (easy_helper::easy_async_send(eio, slave_server_id, packet, this,
                                   &handler) == EASY_ERROR) {
    log_error("[%s] Send ResponseGetServerTablePacket %s failure.", group_name,
              tbsys::CNetUtil::addrToString(slave_server_id).c_str());
    delete packet;
  }
}
...
bool group_info::write_server_table_packet(char *data, int size) {
  ...
  if (server_table_manager.get_size() == size && mdata != NULL &&
    ...
    server_table_manager.sync();
    (*server_table_manager.last_load_config_time) =
        0; // force to reload config file, since the file is changed
    log_info("[%s] accept servertable successed, hashcode: %u, "
             "migrateBlockCount: %d, "
...
void group_info::set_migrating_hashtable(size_t bucket_id, uint64_t server_id) {
  bool ret = false;
  if (bucket_id > server_table_manager.get_server_bucket_count() ||
      is_migrating() == false)
    return;
  if (server_table_manager.m_hash_table[bucket_id] != server_id) {
    log_error("where you got this ? old hashtable? bucket_id: %lu, m_server: "
              "%s, server: %s",
              bucket_id,
              tbsys::CNetUtil::addrToString(
                  server_table_manager.m_hash_table[bucket_id])
                  .c_str(),
              tbsys::CNetUtil::addrToString(server_id).c_str());
    return;
  }
  ...
  if (ret == true) {
    ...
    log_info("setMigratingHashtable bucketNo %lu serverId %s, finish migrate "
             "this bucket.",
...
void group_info::set_recovery_hashtable(size_t bucket_id, uint64_t server_id) {
  if (bucket_id > server_table_manager.get_server_bucket_count() ||
    ...
    log_error("bucket_id invalid or is_recoverying false. bucket_id(%lu), "
              "bucket_num(%d), recovery_block_count(%d)",
  ...
  if (server_table_manager.m_hash_table[bucket_count + bucket_id] != 0 ||
    ...
    log_error("error happen, bucket_id: %lu, server: %s, m_server[0] is %s, "
              "m_server[1] is %s, d_server[0] is %s, d_server[1] is %s",
              bucket_id, tbsys::CNetUtil::addrToString(server_id).c_str(),
              tbsys::CNetUtil::addrToString(
                  server_table_manager.m_hash_table[bucket_count + bucket_id])
                  .c_str(),
              tbsys::CNetUtil::addrToString(
                  server_table_manager.m_hash_table[bucket_id])
                  .c_str(),
  ...
  // once recoveryed one bucket, notice client and force set the m_hashtable of
  // ds
  inc_version(server_table_manager.client_version);
  ...
  log_info("[%s] version changed: clientVersion: %u, serverVersion: %u, "
           ...
           group_name, *server_table_manager.client_version,
           *server_table_manager.server_version,
           *server_table_manager.recovery_version);
  ...
  if (it != recovery_machine.end()) {
    if (it->second > 0) {
      ...
      log_error("error happen, the num of remain recovery bucket in %s is "
                "negative(%d)",
                tbsys::CNetUtil::addrToString(failover_ds).c_str(), it->second);
    }
  } else {
    log_error("error happen, ds %s not in recovery_machine",
              tbsys::CNetUtil::addrToString(failover_ds).c_str());
  }

  log_info("setMigratingHashtable bucketNo %lu serverId %s, finish recovery "
           "this bucket.",
           bucket_id, tbsys::CNetUtil::addrToString(server_id).c_str());
  should_syn_mig_info = true;
...
void group_info::get_migrating_machines(
    vector<pair<uint64_t, uint32_t>> &vec_server_id_count) const {
...
void group_info::inc_transition_version() {
  if (get_transition_version() ==
      cluster_transition_info::MAX_TRANSITION_VERSION) {
    *server_table_manager.transition_version = 0;
  }
...
void group_info::get_stat_info(uint64_t server_id,
  ...
  stat_info_rw_locker.wrlock(); // here we use write lock so no data server can
                                // update its stat info
  if (server_id == 0) {
...
int group_info::add_down_server(uint64_t server_id) {
  ...
  if (sit == tmp_down_server.end()) {
    log_debug("add server: %lu not exist", server_id);
    // add
    ...
    ret = tair::util::file_util::change_conf(group_file_name, group_name,
                                             TAIR_TMP_DOWN_SERVER,
                                             down_server_list.c_str());
    if (ret != TAIR_RETURN_SUCCESS) {
      log_error("change conf failed, ret: %d, group_file: %s, group_name: %s",
                ret, group_file_name, group_name);
    }
  } else // do nothing
  {
    log_debug("add server: %lu exist", server_id);
  }
  return ret;
}
...
int group_info::clear_down_server(const vector<uint64_t> &server_ids) {
  int ret = TAIR_RETURN_SUCCESS;
  log_debug("clear down server. server size: %lu, clear size: %lu",
            tmp_down_server.size(), server_ids.size());
  ...
  if (!tmp_down_server.empty()) {
    ...
    if (server_ids.empty()) { // mean clear all down servers
      ...
      for (vector<uint64_t>::const_iterator it = server_ids.begin();
           it != server_ids.end(); ++it) {
        tmp_down_server.erase(*it);
      }
      if (!tmp_down_server.empty()) {
        down_servers_str.reserve(tmp_down_server.size() * 32);
        for (std::set<uint64_t>::iterator sit = tmp_down_server.begin();
             sit != tmp_down_server.end(); ++sit) {
...
int group_info::get_server_down_time() const { return server_down_time; }
...
void group_info::do_proxy_report(const request_heartbeat &req) {
  if (is_migrating()) {
    ...
    if (it == reported_serverid.end()) {
      reported_serverid.insert(
          req.get_server_id()); // this will clear when migrate complete.
      log_info("insert reported server: %s, bucket_count: %lu",
               tbsys::CNetUtil::addrToString(req.get_server_id()).c_str(),
               req.vec_bucket_no.size());
...
int group_info::select_build_strategy(const node_info_set &ava_server) {
  ...
  for (set<node_info *>::const_iterator it = ava_server.begin();
    ...
    log_info("mask %lu:%s & %lu -->%lu", (*it)->server->server_id,
             tbsys::CNetUtil::addrToString((*it)->server->server_id).c_str(),
             pos_mask, (*it)->server->server_id & pos_mask);
    (pos_count[(*it)->server->server_id & pos_mask])++;
  }

  if (pos_count.size() <= 1) {
    log_warn("pos_count size: %lu, table builder3 use build_strategy 1",
             pos_count.size());
    strategy = 1;
  } else {
    int pos_max = 0;
    ...
    if (ratio > diff_ratio ||
        ava_server.size() < server_table_manager.get_copy_count() ||
        ratio > 0.9999) {
      log_warn(
          "pos_count size: %lu, ratio: %f, table builder3 use build_strategy 1",
          pos_count.size(), ratio);
      strategy = 1;
    } else {
      log_warn(
          "pos_count size: %lu, ratio: %f, table builder3 use build_strategy 2",
          pos_count.size(), ratio);
      strategy = 2;
    }
    log_warn("diff_server = %d ratio = %f diff_ratio=%f", diff_server, ratio,
             diff_ratio);
  }
